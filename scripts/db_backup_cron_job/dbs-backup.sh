...
PG_DUMP=/backup-data/db-backup.sql
REDIS_DUMP=/backup-data/dump.rdb
NUM_BACKUP_KEEP=5

echo "Checking object storage..."
if [ "$STORAGE_TYPE" == "minio" ] || [ "$STORAGE_TYPE" == "aws" ]; then
    printf "Object storage type is ${STORAGE_TYPE}, continuing with backup.\n"
else
    printf "Object storage type is ${STORAGE_TYPE}, which isn't supported.\nExitting backup plan.\n"
    exit 1
fi

echo "Connecting to object storage..."
mc alias set $STORAGE_TYPE $STORAGE_ENDPOINT $STORAGE_ACCESS_KEY $STORAGE_SECRET_KEY --api S3v4
test_bucket=$(mc ls $STORAGE_TYPE/$STORAGE_BUCKET | wc -l)
if [ $test_bucket -gt 0 ]
then
...
echo $PG_PASSWORD | pg_dump -h postgres -U dbuser -d production -Fc -v > $PG_DUMP
...
mc cp $REDIS_TAR $STORAGE_TYPE/$STORAGE_BUCKET/$REDIS_TAR
mc cp $PG_TAR $STORAGE_TYPE/$STORAGE_BUCKET/$PG_TAR

echo "Number of backups to keep is set to:" ${NUM_BACKUP_KEEP}
echo "Checking if older backups need to be deleted..."

CURRENT_PG_NUM=$(mc ls $STORAGE_TYPE/$STORAGE_BUCKET | grep pg-backup | wc -l)
CURRENT_REDIS_NUM=$(mc ls $STORAGE_TYPE/$STORAGE_BUCKET | grep redis-backup | wc -l)
...
do
    PG_OLDEST_BACKUP=$(mc ls $STORAGE_TYPE/$STORAGE_BUCKET | grep pg-backup | sort | head -n 1 | awk '{print $6}')
    echo "Deleting the oldest backup" $PG_OLDEST_BACKUP
    mc rm $STORAGE_TYPE/$STORAGE_BUCKET/$PG_OLDEST_BACKUP
    CURRENT_PG_NUM=$(mc ls $STORAGE_TYPE/$STORAGE_BUCKET | grep pg-backup | wc -l)
done
...
do
    REDIS_OLDEST_BACKUP=$(mc ls $STORAGE_TYPE/$STORAGE_BUCKET | grep redis-backup | sort | head -n 1 | awk '{print $6}')
    echo "Deleting the oldest backup" $REDIS_OLDEST_BACKUP
    mc rm $STORAGE_TYPE/$STORAGE_BUCKET/$REDIS_OLDEST_BACKUP
    CURRENT_REDIS_NUM=$(mc ls $STORAGE_TYPE/$STORAGE_BUCKET | grep redis-backup | wc -l)
done
...
echo "Postgres"
mc ls $STORAGE_TYPE/$STORAGE_BUCKET | grep pg-backup | awk '{print $6}'
echo ""
echo "Redis"
mc ls $STORAGE_TYPE/$STORAGE_BUCKET | grep redis-backup | awk '{print $6}'

echo ""
echo "Backup finished:"
mc ls $STORAGE_TYPE/$STORAGE_BUCKET/$REDIS_TAR
mc ls $STORAGE_TYPE/$STORAGE_BUCKET/$PG_TAR
